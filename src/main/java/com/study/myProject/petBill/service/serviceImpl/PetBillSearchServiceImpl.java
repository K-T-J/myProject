...
public class PetBillSearchServiceImpl implements PetBillSearchService {
	...
	@Override
	public Map<String, Object> getHosList(String pageNum) {
		...
		List<PetBillHospitalDTO> hosList = null;	// 전체(검색 또는 선택X) 게시글들 담아줄 변수 
...
//		result.put("revCount", revCount);
	...
	@Override
	public Map<String, Object> getSearchHosList(String pageNum, String search) {
		// ** 게시글 페이지 관련 정보 세팅 **
		// 한페이지에 보여줄 병원 list 수 
		int pageSize = 10;
		// 현재 페이지 번호 설정해주기. 파라미터 값 없으면 1로 설정. 
...
//		System.out.println("2");
		// 지역함수 외에 사용하기 위해 변수 생성 
//		List<PetBillRevCountDTO> hosList = null;	// 전체(검색 또는 선택X) 게시글들 담아줄 변수 
//		int count = 0;						// 전체(검색 또는 선택X) 글의 개수 
//		int number = 0;						// 화면에 뿌려줄 가상 글 번호 
...
//		searchDAO.getSelSearchList(sidoSel, siguSel, search, startRow, endRow);
//		
//		// 검색한 병원 목록에 뿌려줄 가상의 글 번호 
//		number = count - (currentPage - 1) * pageSize;	
//		System.out.println("시/도,구O(부/자) number : " + number);
